...
windows Native Peing Connector(NPC)は、Twitter質問箱の愛称で親しまれていたウェブサービス「Peing」(https://peing.net/)をWindowsのGUIから便利に利用するためのツールです。

このサービスでは、ウェブ上で質問箱を開設したユーザに対して、匿名で質問を送ることができます。
質問を受け取った開設者は、公開の場で回答します。
NPCでは、このサービスが提供する機能の内、回答済みの質問の閲覧、匿名またはログイン状態での質問投稿、届いた質問への回答などの主要機能をwindowsのGUI上から簡単な操作で行うことができます。

閲覧の為に受信した回答のデータはお使いのPCに保存します。回答者がアカウントや回答を削除した場合にも、手元にデータを残すことができます。
さらに、万一Peingのサービスが終了した場合に備えたバックアップとしても機能します。
本ソフトは、サービス終了までの間にデータを蓄積していた場合には、そのデータのビューアーとして動作し続けます。
ただし、最新データの取得ができなくなるとともに、届いた質問・アーカイブ済みの質問の表示機能についてはデータの保存をせずリアルタイム表示する仕様のため、サービス終了後は閲覧できなくなることにご注意ください。基本的には、自分自身のアカウントもデータ取得対象として追加しておくことを想定しています。

なお、サービスへのアクセスは通信の解析により運営者の許諾を得ることなく実現されているため、将来的な利用は保証されていません。
...
この操作を登録したいアカウントの数だけ繰り返します。一括登録する機能については後述します。
...
なお、４．４．１．２．４においてログイン状態で質問するよう設定していた場合には、ログインのため、投稿に少し時間がかかる場合があります。
...
[このユーザのみ表示]ボタンを押すと、４．２．３で後述する[選択中ユーザのみ表示]のフィルタを当該ユーザに対して適用してダイアログを閉じます。
[削除]ボタンまたは[delete]キーを押すと、４．１．４と同様に当該ユーザの登録を削除します。
[質問を投稿]ボタンを押すと、４．１．２と同様に当該ユーザへ質問を送ることができます。
[絞り込み]エディットボックスにアカウント名または表示名の一部、もしくは@に続けてアカウント名の先頭部分を入力すると、表示するアカウントを絞り込むことができます。
リストビューの上でコンテキストメニューを表示すると、各ボタンと同じ操作を行うことができます。また、リスト上の２つのカラムの表示順序を入れ替えることができます。なお、この並び替えを行った際にPC-Talkerでの読み上げがおかしくなった場合は、一旦ダイアログを閉じてから開き直してください。リストの順序は保存されます。

リストビューで任意のユーザにカーソルを合わせたうえでコンテキストメニューを開くと、「強制再受診」を行うことができます。
この機能はVersion1.4.1以前の利用者で、回答データが途中までしか取得できていなかった場合に不足分のダウンロードを試みるために用意されています。実行すると、過去の回答を全件再受診し、手元のデータにないものがあれば登録します。
この機能を用いても、すべてのデータを受信できることを保証するものではありません。

４．１．10　ユーザの一括追加
複数のユーザを一括で追加することができます。
画面の指示に従い、追加したいユーザのアカウントを下位行区切りで入力してください。
...
４．１．11　ユーザの回答数の不整合を確認

ユーザの回答数と、手元で保有している回答データの数を比較して確認します。
後者の方が少ない場合に、外套のユーザー名と回答数、手元のデータの数を表示します。
Version1.4.1以前の利用者を対象に、回答データを途中までしか受信できていなかったケースに該当しているか否かを確認するために用意されている機能です。

４．１．12　終了　「Alt+F4」
NPCを終了します。

４．２　フィルタ
フィルタ機能を利用すると、ビューに表示する回答データを絞り込むことができます。
フィルタを重複して適用すると、適用中の全てのフィルタの条件を満たした回答のみが表示(AND条件で判定)されます。
...
４．２．４　指定条件で検索して表示
指定したキーワードを含む回答のみを表示することができます。
この項目を実行すると、[回答データの検索]ダイアログが開きます。開いたダイアログで、キーワード、検索対象(質問のみ、回答のみ、両方)、キーワードを正規表現として扱うか否かを設定します。
[OK]を押してダイアログを閉じると、ビューが更新されて条件に合う回答のみが表示された状態になります。
もう一度この項目を実行すると、このフィルタを解除することができます。

４．２．５　すべてのフィルタを解除
適用中のフィルタをまとめて解除することができます。


４．３　アカウント
このメニューには、アカウントへのログインを行ったうえで利用する機能が集められています。
利用する前に、４．４．１．２で説明する設定機能を用いて、PeingのIDとパスワードを設定しておく必要があります。
...
[アーカイブ]ボタンをまたは[delete]キーを押すと、カーソルを合わせた質問をアーカイブすることができます。

４．３．２　アーカイブ済みの質問を表示
過去にアーカイブした質問を一覧表示し、回答または元に戻す操作ができます。
ダイアログ内の操作は４．３．１のものとほとんど同じです。
...
[運営からの自動質問]では、運営から自動で届く質問の受信をするか否かを設定することができます。
...
４．４．１．１．２　次回起動時にフィルタの状況を維持
NPCを終了して再度起動した際、一部のフィルタの設定状況を引き継ぐか否かを設定します。
この項目をチェックすると、４．２．１で解説した[運営からの質問を非表示]と、４．２．２で解説した[バトン質問を非表示]の２つのフィルタの設定状態を次回起動時に引き継ぐことができます。
その他のフィルタは引継ぎ対象にならない点に注意してください。
既定ではチェックされていません。

４．４．１．１．３　次回起動時にカーソルの位置を復元
NPCを終了して再度起動した際、最後に閲覧していた質問にカーソルを合わせた状態で起動するか否かを設定します。
既定でチェックされています。
...
４．４．１．２　ログイン
ログインが必要な機能を利用するための設定です。
ログイン機能の利用には、幾つかの注意事項があります。５．１の内容をお読みになったうえで使用してください。

４．４．１．２．１　利用するIDの種類
Peingへのログインに用いるアカウントをTwitterIDまたはPeingIDから選択することができる機能でしたが、TwitterIDでのログイン機能は廃止されました。

４．４．１．２．２　ID
ログインが必要な機能を利用する際に用いるPeingアカウントのIDを設定します。
PeingIDまたは登録メールアドレスを入力します。
入力したIDを用いてPeingにログインしたことがある必要があります。NPCは初回ログインおよびIDの新規登録には利用できません。
なお、Twitterからの連携でPeingアカウントを作成した場合のPeingIDは、Twitterのアカウント名の先頭の@を消し、全ての英字を小文字にしたものとなります。
設定を解除する場合には、IDと次の項目で詳述するパスワードを両方とも空欄としてください。

４．４．１．２．３　パスワード
IDと併せてPeingのログインに用いるパスワードを設定します。
なお、Twitterを含むSNSからの連携でPeingアカウントを作成した場合には、PeingIDのパスワードが設定されていない状態となります(2021年４月現在)。
その場合には、ここで設定を行う前に、ウェブや公式アプリからPeingにログインしてパスワードの設定を行ってください。
設定を解除する場合には、IDとパスワードを両方とも空欄としてください。

４．４．１．２．４　ログインした状態で質問する
質問の投稿の際に、ログインした状態で行うか否かを設定します。
ログインした状態で質問を行うことで、４．３．３で詳述した[送信した質問の表示]機能が利用できるようになります。
既定ではチェックされていません。
...
４．４．１．３．４　複数行の質問・回答を入力可能にする
質問の投稿、届いた質問への回答時に、複数行の入力を可能にするかどうかを設定します。
この設定をチェックした場合、入力欄でEnterキーを押すことで、改行を挿入できるようになります。
規定ではチェックされていません。
...
３．　４．４．１．２．２および４．４．１．２．３で設定したPeingのIDおよびパスワードは、「settings.ini」というファイル名で他の設定内容と併せて暗号化することなく保存されます。取り扱いには十分に注意してください。
４．　ログイン機能は、公式に認められていない方法で実装しています。そのため、突然利用できなくなったり、不審な行動と判断されて利用しているアカウントの停止等の措置を取られてしまう場合もあります。
　　　これらの事態が発生した場合であっても、ライセンス条項記載の通り開発者及び配布元は一切の補償を行うことができませんので、よくご理解いただいたうえでご利用ください。
５．　NPCの使用中、以下のようなメッセージが表示されることがあります。
----------
